library(tidyverse)

full_data %>%
  mutate(SWC = soil_vwc_15cm) %>%
  mutate(SEC = soil_ec_15cm) %>%
  dplyr::select(Year, Species, ID, TIMESTAMP, Plot, `F`, PAR, SWC, SEC) %>%
  mutate(Date = as_date(date(TIMESTAMP)),
         Hour = hour(TIMESTAMP)) %>%
  left_join(events) %>%
  group_by(Year) %>%
  mutate(data_start = flood_start - window,
         data_end = flood_end + window) %>%
  filter(Date > data_start & Date < flood_start |
         Date < data_end & Date > flood_end,
         Hour < 14, Hour >= 11, 
         Plot != "Freshwater",
         Species == "Tulip Poplar") %>%
  ungroup() -> tulip_all

tulip_all %>%
  group_by(Year) %>%
  mutate(BA = if_else(Date > flood_end, "After", "Before"),
         BA = if_else(Year == 2021, "Before", BA),
         Year = as.factor(Year),
         ID = as.factor(ID),
         BA = as.factor(BA),
         Plot = as.factor(Plot)) %>%
  filter(`F` < 4e-06, 
         SWC > 0.12,
         SWC < 0.6,
         (PAR > 480 | is.na(PAR))) %>%
  group_by(Date, Plot, ID, Year, BA, flood_start, flood_end) %>%
  summarise(F_avg = mean(`F`, na.rm = TRUE),
            n = n(), 
            soil_ec_avg = mean(SEC), 
            soil_vwc_avg = mean(SWC)) %>%
  ungroup() -> tsb_4

tsb_4 %>%
  mutate(BA = fct_relevel(BA, "Before", "After")) -> tsb_4

ggplot(tulip_all[tulip_all$Year == 2021,], aes(SEC, SWC, color = Plot)) +
  geom_point() + scale_color_viridis_d() +
  ylim(0, 0.4) +
  stat_smooth(method='lm') + theme_light()


tsb_4 %>%
  filter(Year == 2021) %>%
ggplot(aes(soil_ec_avg, F_avg)) +
  geom_point(aes(color = soil_vwc_avg)) + scale_color_viridis_c() +
  stat_smooth(method = 'lm', color = 'black',
              size = 1) +
  stat_cor(label.y = 2.2e-06) +
  stat_regline_equation(label.y = 2.0e-06) +
  facet_wrap(.~Plot, scales = "free_x") +
  ggtitle("Pre-Treatment EC vs Sapflow") +
  labs(color = "VWC", y = "Sapflow", x = "Soil EC") +
  theme_light()

tsb_4 %>%
  #filter(soil_ec_avg < 1300) %>%
  ggplot(aes(soil_ec_avg, F_avg)) +
  geom_point(aes(color = Year)) + scale_color_viridis_d() +
  stat_smooth(aes(group = BA), method = 'lm', color = 'black',
              size = 1) +
  stat_cor(label.y = 2.2e-06) +
  stat_regline_equation(label.y = 2.0e-06) +
  facet_wrap(Plot~BA, scales = "free", ncol = 2) +
  theme_light()

tsb_4 %>%
  group_by(Year, Plot, BA) %>%
  ggplot(aes(Year, F_avg, fill = interaction(BA, Plot))) +
  scale_fill_viridis_d() +
  geom_boxplot() +
  ggtitle("Plot Sapflow") +
  labs(fill = "BACI", y = "Sapflow", x = "Year") +
  theme_light()
