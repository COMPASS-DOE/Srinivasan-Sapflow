...
ggplot(par_2, aes(Day, par_avg, shape = BA)) +
  geom_point(size = 2.5) + facet_grid("Year", scales = "free")
```
...
Neither the likelihood ratio test nor the parametric bootstrap test show significance for the BACI interaction term alone.
...
```
Neither the liklihood ratio test and parametric bootstrap test show statistically significance for the effect of the BACI interaction on EC. 
