...
ggplot(sf_plot_avg) + 
  ...
  theme_classic() +
  labs(y = "Avg Sap Flux Density x 10^6", x = "Date",
...
data %>%
  dplyr::select(Year, Species, ID, TIMESTAMP, Plot, Fd_scaled,
...
tulip_salt %>%
  ...
  summarise(F_avg = mean(Fd_scaled, na.rm = TRUE),
...
tsb_2_summary %>%
  ...
  labs(y = expression ("Midday Sap Flow, 10"^6*" m"^3* " s"^-1),
       x = expression(paste("Mean Soil EC, ", mu, "S cm"^-1)),
       color = " ") +
  theme(legend.position="none", element_text(size = 14))
```
