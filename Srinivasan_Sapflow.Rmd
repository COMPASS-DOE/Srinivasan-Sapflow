...
for (species in species_list) {
 ...
 # ---- NEW: save with a descriptive filename ----
    # (no extra packages; just base R string fixes)
    species_safe <- gsub("[^A-Za-z0-9_-]", "_", species)
    plot_safe    <- gsub("[^A-Za-z0-9_-]", "_", plot)
    ts           <- format(Sys.time(), "%Y%m%d_%H%M%S")  # optional timestamp

    fname_png <- file.path(overview_plots, paste0("BACI_", species_safe, "_", plot_safe, "_", ts, ".png"))
...
emm_3way <- emm_3way %>%
  mutate(
    ...
    dodge = ifelse(Year == levels(Year)[1], -0.35, 0.2)
  )
