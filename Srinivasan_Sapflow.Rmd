...
data %>%
  dplyr::select(Year, Species, ID, TIMESTAMP, Plot, Fd,
...
tulip_salt %>%
  ...
  summarise(F_avg = mean(Fd, na.rm = TRUE),
...
tsb_1 %>%
  ...
  mutate(Day = ifelse(BA == "Before",
         ...
         BA = factor(BA, levels = c("Before", "After"))) -> tsb_2
...
Choose the model with the lowest AICc value (i.e., smallest positive or largest negative); this suggests that the second model, which includes the random effects interaction term, is a better fit for our data.

#Create and test BACI model(s)

Our existing model- including only Plot and BA as fixed effects: 
...
We significance in the BACI interaction term, indicating that our treatment has had an impact on sapflow.
...
We now see the BACI interaction term has strong explanatory power for our data with the inclusion of soil ec as fixed effect in both the likelihood ratio and parametric bootstrap 
...
AIC %>%
  mutate(`Model Specification` = candidate,
         AIC_rounded = round(AIC)) %>%  # Round AIC values
  ggplot(aes(variable, AIC, fill = `Model Specification`)) +
  geom_col() +
  geom_hline(yintercept = 3675, linetype = "dashed") +
  geom_text(aes(label = AIC_rounded),  # Add text annotations
            vjust = -0.5,              # Adjust position above the bar
...
tsb_2_summary %>%
  ...
  geom_errorbarh(aes(xmin = mean_annual_EC - sd_annual_EC,
                    ...
                    y = mean_annual_F,
                    color = Year), size = 1) +
  stat_smooth(aes(x = soil_ec_avg, y = F_avg, group = BA), method = 'lm',
              color = 'black', size = 1,
...
# Summarize again for annual means and standard deviations (if needed)
hourly_soil_data_summary <- hourly_soil_data %>%
  group_by(Plot, BA, Year) %>%
  summarise(mean_annual_F = mean(soil_vwc_avg),
            sd_annual_F = sd(soil_vwc_avg),
            mean_annual_EC = mean(soil_ec_avg),
            sd_annual_EC = sd(soil_ec_avg),
            n = n(), .groups = 'drop')
...
ggplot() +
  geom_point(data = hourly_soil_data_summary,
             aes(mean_annual_EC, mean_annual_F,
                 color = as.factor(Year)), size = 3) +
  geom_errorbar(data = hourly_soil_data_summary,
                aes(x = mean_annual_EC,
                    ymin = mean_annual_F - sd_annual_F,
                    ymax = mean_annual_F + sd_annual_F,
                    color = as.factor(Year)), size = 0.8) +
  geom_errorbarh(data = hourly_soil_data_summary,
                 aes(xmin = mean_annual_EC - sd_annual_EC,
                     xmax = mean_annual_EC + sd_annual_EC,
                     y = mean_annual_F,
                     color = as.factor(Year)), size = 0.8) +
  geom_point(data = hourly_soil_data %>% filter(soil_ec_avg < 1250),
             aes(x = soil_ec_avg,
                 y = soil_vwc_avg,
                 color = as.factor(Year)),
             alpha = 0.3) +
  stat_smooth(data = hourly_soil_data %>% filter(soil_ec_avg < 1250),
              aes(x = soil_ec_avg, y = soil_vwc_avg, group = Year,
                  color = Year),
              method = 'lm', se = FALSE,
              size = 1, alpha = 0.5 ) +
  # stat_regline_equation(data = hourly_soil_data,
  #                       aes(x = soil_ec_avg, y = soil_vwc_avg,
  #                           label = ..eq.label..),
  #                       size = 3.5, color = "black") +
  # stat_cor(data = hourly_soil_data,
  #          aes(x = soil_ec_avg, y = soil_vwc_avg,
  ...
  theme_classic() +
  labs(y = expression ("Mean Soil VWC, m" ^3* " m"^-3),
       x = expression(paste("Mean Soil EC, ", mu, "S cm"^-1)),
       color = " ")
