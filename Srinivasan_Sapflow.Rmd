...
tulip_salt %>%
  ...
  summarise(F_avg = mean(`F`, na.rm = TRUE),
            soil_ec_avg = mean(ec_avg, na.rm = TRUE), 
...
tsb_2_summary = tsb_2 %>%
  filter(ifelse(BA == "Before", soil_ec_avg < 250, soil_ec_avg < 1000)) %>%
  ...
  summarise(mean_annual_F = mean(F_avg),
            ...
            n = n())
#tsb_2 %>%
#  filter(!(BA == "Before" & soil_ec_avg > 750)) -> tsb_2
...
^Keeping original analysis & plots above for reference, but can remove/modify later. 

2021 vs 2025 BACI Analysis: 
...
all_1 %>%
  ...
  mutate(BA = as.character(BA),
         BA = ifelse(Year == 2021, "Before", BA),
         BA = ifelse(Year == 2025, "After", BA),
         BA = as.factor(BA)) -> all_2
```

```{r}
model.int.all <- lmer(F_avg ~ BA*Plot +
                        (1|ID) + (1|Year) + (1|ID:Year),
                      data = all_2)

model.noint.all <- lmer(F_avg ~ BA + Plot +
                          (1|ID) + (1|Year) + (1|ID:Year),
                        data = all_2)

all.refdist <- PBrefdist(largeModel = model.int.og, 
                        smallModel = model.noint.og, 
                        nsim = 100)

all.compar <- PBmodcomp(largeModel = model.int.og, 
                       smallModel = model.noint.og,
                       ref = og.refdist)
