...
#Following unzip, source files create_inventory.R and frankenstein.R
...
for (species in species_list) {
 ...
 #save each plot with descriptive file name
    species_safe <- gsub("[^A-Za-z0-9_-]", "_", species)
    plot_safe    <- gsub("[^A-Za-z0-9_-]", "_", plot)
    ts           <- format(Sys.time(), "%Y%m%d_%H%M%S")  # optional timestamp

    fname_png <- file.path(overview_plots,
                           paste0("BACI_",
                                  species_safe, "_",
                                  plot_safe, "_", ts,
                                  ".png"))
    ggsave(filename = fname_png, plot = p, width = 10, height = 6, dpi = 300)
  }
}

#based on these plots excluding the following trees:
# C8, C19 (died during the experiment)
```
...
```{r plot Long-Term BACI, fig.width=15, fig.height=10, out.width="100%"}
...
emm_3way <- as.data.frame(cld_species)

# for dodging letters below
emm_3way <- emm_3way %>%
  mutate(
    Year_num = as.numeric(as.character(Year)),
    dodge = ifelse(Year == levels(Year)[1], -0.5, 0.5)
  )

#plot of 21 vs 25 EMM with 95% CI
ggplot(emm_3way, aes(x = Year_num, y = emmean, color = Plot, shape = Plot)) +
  facet_grid(~ Species) +
  ...
  geom_line(aes(group = interaction(Species, Plot)),
            ...
            position = position_dodge(width = 0.5)) +
  geom_text_repel(aes(label = .group, x = Year_num + dodge),
            vjust = -0.5, size = 6, show.legend =FALSE) +
  scale_x_continuous(
    breaks = sort(unique(emm_3way$Year_num)),
    labels = sort(unique(emm_3way$Year))
  ) +
  scale_color_viridis_d(option = "mako", begin = 0.3, end = 0.8) +
  labs(
    y = "EMM Sap Flux (cm³ cm⁻² s⁻¹)",
    x = "Year",
...
model.int.og <- lme4::lmer(F_avg ~ BA*Plot +
                        (1|ID) + (1|Year) + (1|ID:Year),
                      data = tsb_2 )

model.noint.og <- lme4::lmer(F_avg ~ BA + Plot +
                          (1|ID) + (1|Year) + (1|ID:Year),
                        data = tsb_2 )
...
The likelihood ratio test indicates little effect of the BACI interaction term.
...
model.int.ec <- lme4::lmer(F_avg ~ BA*Plot + soil_ec_avg +
                      (1|ID) + (1|Year) + (1|ID:Year),
                      data = tsb_2 )

model.noint.ec <- lme4::lmer(F_avg ~ BA + Plot + soil_ec_avg +
                        (1|ID) + (1|Year) + (1|ID:Year),
                        data = tsb_2 )
...
tsb_2 %>%
  mutate(
    ...
    Plot_BA = interaction(Plot, BA, drop = TRUE)  # 4 unique combos
  ) %>%
  ggplot(aes(x = Plot, y = F_avg, fill = Plot_BA)) +
  geom_boxplot() +
  facet_wrap(Year ~ .) +
  scale_fill_viridis_d(begin = 0.1, end = 0.9) +
  labs(y = "Midday Sap Flow (cm³ cm⁻² s⁻¹)", x = NULL) +
  theme_bw() +
  theme(
  ...
  guides(
    color   = guide_legend(title = "Plot × BA"),  # 4-color legend
...
plot_data %>%
  ...
  arrange(Date) %>%
  mutate(f_roll = zoo::rollmean(F_avg, 100, na.pad = TRUE,                                align = "center")) -> rolled_data
...
try <- mixed(F_avg ~ Year*Plot*Species + (1|ID), data = final_BACI_data)
try
try2 <- mixed(F_avg ~ BA*Plot + soil_ec_avg + (1|ID) + (1|Year) + (1|ID:Year), data = tsb_2)
try2
```
