...
This is a summary of the pipeline I have used over the past summer analyzing TEMPEST sap flow data. 
...
Sap flow velocity would be the term to describe the point/unscaled sap flow measurement, and sap flux density would describe sap flow over a certain area (in this case, sapwood area of the tree).
...
Specify start and end times for data (here, we focus on the last two weeks of April 2024): Note: Have used EST to avoid defaulting to UTC, not sure if it is appropriate time zone.
...
Calculate dTmax (the maximum difference in voltage, i.e. the minimum sapflow) over a 24 hour period. Note: This value was initially calculated between the hours of midnight and 5 am, but this failed to account for instances of low sapflow during the day. Perhaps averaging this value over a 7-10 day period would be more accurate?
...
Convert voltage difference to sap flux density ($F_d$,$m^3/m^2 \cdot s$) using Granier 1985 equation, then convert to $g/m^2 \cdot hr$.

$$ F_{d}  = \alpha K^{b} $$ Constants alpha and b were experimentally determined by Granier and depend on the power of the heat probe (0.2 W).

$$ F_{d}  = 118.99 \cdot 10^{-6} \cdot (\frac{\Delta T_{max}}{\Delta T}- 1)^{1.231}$$ Scale to get to g/m2/s, $$ F_{d}  =  360,000 \cdot F_{d}$$
