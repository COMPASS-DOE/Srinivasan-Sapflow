...
Graph whole tree flux for 9-10 AM over the last two weeks of April, averaged by plot and species. 
Note: These are averaged values for every 15 minutes from 9-10 AM. 
```{r}
scaled %>% 
  filter(Hour >= 9, Hour <= 10) %>% 
  group_by(Plot, TIMESTAMP, Species) %>% 
  summarise(Ftot_avg = mean(F_tot, na.rm = TRUE)) -> scaled_plot_avg

ggplot(scaled_plot_avg) + 
  geom_line(aes (x = TIMESTAMP, y = Ftot_avg, color = Species)) + 
  facet_wrap(~Plot, ncol = 1)
  ggsave("weeklyplot.jpeg")
```
...
# 
# ggplot(sfd_plot_avg) + 
#   geom_point(aes (x = factor(TIMESTAMP), y = Fd_avg, color = Species)) + 
#   facet_wrap(~Plot, ncol = 1) +
#   scale_x_discrete(breaks = levels(TIMESTAMP)[c(T, rep(F, 9))])
...
```{r}
sfd_data %>%
  filter(Hour <= 10, Hour >= 10) -> sfd_data
