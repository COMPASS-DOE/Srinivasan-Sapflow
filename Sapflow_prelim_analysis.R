...
#tidy up data
#grepl!! Steph you're a genius!! I was doing it the long way!!
...
sapflow_data %>% 
  drop_na(ID) %>%
  select(TIMESTAMP, Date, Hour, ID, Species, Plot.y, Location, Value) %>% 
...
#Fd is sap flux density, unclear on relation to velocity/other metrics of flow? 
sf_dat %>% 
...
sfd_data %>% 
  filter(Date <= "2024-04-22", Hour >= 5, Hour < 21) %>% 
  group_by(Plot.y, TIMESTAMP) %>% 
  summarise(datetime_est = mean(TIMESTAMP, na.rm = TRUE), 
            Fd_avg = mean(Fd, na.rm = TRUE)) %>% 
  relocate(datetime_est) %>% 
  mutate(Fd_avg = round(Fd_avg, digits = 3)) -> sfd_plot_avg

ggplot(sfd_plot_avg) + 
  geom_line(aes (x = TIMESTAMP, y = Fd_avg, color = Plot.y)) + 
  facet_wrap(~Plot.y, ncol = 1) +
