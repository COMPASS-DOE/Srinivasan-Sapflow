...
#making out data frame just a little smaller
filtered_data %>%
...
#calculates tests of 2021 vs 2025 data for each species x plot group
contrasts <- contrast(model_emm, interaction = "pairwise", by = c("Species", "Plot"), adjust = "holm")
...
ggplot(emm_3way, aes(x = Year, y = emmean, color = Plot)) +
  ...
  geom_text(aes(label = .group),
            position = position_dodge(width = 0.4), vjust = -0.9, size = 5) +
    scale_color_viridis_d(option = "mako",
                        begin = 0.3,
                        end = 0.7) + theme_bw() +
  theme_minimal() +
  labs(
    y = "Estimated Marginal Mean (Sap Flux)",
    x = "Year",
