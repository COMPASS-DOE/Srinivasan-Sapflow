...
Further tidy dataframe for BACI analysis; filter for outliers and confirm with plotting. 
```{r}
tulip_salt %>%
  group_by(Year) %>%
...
Visualize all data as part of BACI model:       
```{r}
tsb_1 %>%
  group_by(Year) %>%
...
AIC.res.table
```
Choose the model with the lowest AICc value; this suggests that the second model, which includes the random effects interaction term, is a better fit for our data.
