...
```{r message = FALSE}
.packages = c("tidyr", "ggplot2", "dplyr", "lme4", "AICcmodavg", "MuMIn", "pbkrtest", "readr", "lubridate", "car", "parallel", "data.table", "blmeco", "lsmeans", "patchwork", "viridis")
...
AIC <- c(AIC_int, AIC_noint, AIC_int_ec, AIC_noint_ec, AIC_int_vwc, AIC_noint_vwc)
...
tsb_4 %>%
  ...
  facet_wrap(~ BA, scales = "free", ncol = 2) +
  theme_light() +
  labs(title = "Sap Flow vs Soil EC in Saltwater Plot")
```
```{r}
